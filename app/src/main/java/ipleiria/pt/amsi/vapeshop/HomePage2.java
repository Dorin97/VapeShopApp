...
public class HomePage2 extends AppCompatActivity implements BottomNavigationView.OnNavigationItemSelectedListener, ProdutosListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //lvListaLivro.setAdapter(new ListaLivroAdaptador(this,listaLivros));

        lvListaProduto.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

                Product tempLivro = (Product) parent.getItemAtPosition(position);

                Intent intent = new Intent(getApplicationContext(), DetalhesProduto.class);

                intent.putExtra(DETALHES_PRODUTO, tempLivro.getId());

                startActivity(intent);
                Toast.makeText(HomePage2.this, "-->"+position, Toast.LENGTH_SHORT).show();
            }
        });
    }

    public void onClickAdicionarLivros(View view) {
